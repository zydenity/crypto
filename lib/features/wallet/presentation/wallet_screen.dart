...
import '../../trade/ai_trade_screen.dart';
...
class _WalletScreenState extends State<WalletScreen>
  ...
  Future<void> _refreshBalance() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        _available = asDouble(res['verified'] ?? res['available'] ?? 0);
  ...
  Future<void> _loadHistory() async {
    ...
    try {
      ...
      final transfers =
      _asMapList(results[0]).map(_HistItem.fromTransfer).toList();

      final bankTx =
      _asMapList(results[1]).map(_HistItem.fromBankTransfer).toList();

      final deposits =
      _asMapList(results[2]).map(_HistItem.fromDeposit).toList();
    ...
    final ok = await showModalBottomSheet<bool>(
      ...
      builder: (ctx) {
        ...
        bool validRecipient(String s) {
          ...
          if (RegExp(r'^T[1-9A-HJ-NP-Za-km-z]{25,34}$').hasMatch(t)) {
            return true; // TRON (rough)
          }
          return t.length >= 8;
        }
              ...
              child: Column(
                ...
                children: [
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    items: const [
                      DropdownMenuItem(
                          value: 'ethereum', child: Text('Ethereum (ERC20)')),
                      DropdownMenuItem(
                          value: 'tron', child: Text('TRON (TRC20)')),
                      DropdownMenuItem(
                          value: 'bsc', child: Text('BSC (BEP20)')),
                    ],
                      ...
                      try {
                        ...
                        if (context.mounted) {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                                content: Text(
                                    'Transfer created: ${_fmtExact(amount)} USDT → ${to.substring(0, 6)}…')),
                          );
                        }
                      } catch (e) {
                        setM(() {
  ...
  Future<void> _openBankTransfer() async {
    ...
    try {
      ...
      banks = (results[0] as List).where((e) => e is Map).map((e) {
    ...
    String? bankCode = banks.isNotEmpty ? banks.first['code'] as String : null;
    ...
    await showModalBottomSheet<void>(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setM) {
          ...
          final double phpNet =
          (phpGross - phpFees).clamp(0.0, double.infinity).toDouble();
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    helperText:
                    'Spendable: ${_fmtExact(_available)}  •  Rate: ${useRate.toStringAsFixed(2)}/USDT',
                    prefixIcon: const Icon(Icons.attach_money_rounded),
                  ),
                ),
                const SizedBox(height: 12),
                      ...
                      children: [
                        ...
                        Row(
                            ...
                            children: [
                              const Text('USDT → PHP'),
                              Text(
                                  '${amt.toStringAsFixed(2)} × ${useRate.toStringAsFixed(2)} = ${php(phpGross)}'),
                            ]),
                        Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                ...
                FilledButton(
                  onPressed: sending
                    ...
                    if (bankCode == null ||
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                            content: Text('Fill in all bank details')),
                      );
                      return;
                    }
                    if (toSend <= 0 || toSend > _available) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                            content: Text(
                                'Invalid amount. Spendable: ${_fmtExact(_available)}')),
                      );
                      return;
                    }
                    setM(() => sending = true);
                    try {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                            content: Text(
                                'Transfer queued: ${_fmtExact(toSend)} USDT → $bankCode')),
                      );
            ...
            actions: [
              ...
              Padding(
                ...
                child: FilledButton.tonal(
                  onPressed: null, // disables the button (no edit/change possible)
              ...
              child: Column(
                children: [
                  ...
                  QuickActionsGrid(actions: [
                    QuickAction('Send', FontAwesomeIcons.paperPlane,
                        AppColors.accent, () {
                          if (_sendBusy) return;
                          _openSend();
                        }),
                    QuickAction('Bank Transfer', FontAwesomeIcons.downLong,
                        AppColors.primary, () {
                          if (_sendBusy) return;
                          _openBankTransfer();
                        }),
                    QuickAction('AI Trade', FontAwesomeIcons.rightLeft,
                        AppColors.primary, () {
                          Navigator.of(context).push(MaterialPageRoute(
                              builder: (_) => const AiTradeScreen()));
                        }),
                    // ↓ Navigate to the new Referral page
                    QuickAction('Referral', FontAwesomeIcons.gift,
                        AppColors.accent, () {
                          Navigator.of(context).push(MaterialPageRoute(
                  ...
                  if (_loadingHistory)
                    ...
                    ListView.separated(
                      ...
                      itemBuilder: (_, i) {
                        final h = _history[i];
                        final amtStr =
                            '${h.isOut ? '-' : '+'} ${_fmtExact(h.amount)} ${h.token}';
                        return ListTile(
                          ...
                          subtitle: Row(
                            children: [
                              ...
                              Expanded(
                                child: Text(
                                    '${h.subtitle} • ${_fmtDate(h.createdAt)}',
                                    maxLines: 1,
                                    overflow: TextOverflow.ellipsis,
                                    style: TextStyle(
